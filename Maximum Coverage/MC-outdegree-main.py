...
class ObjectiveMC(object):
    ...
    def InitDVC(self, n, q):
        ...
        for i in range(self.n):
            ...
            if tempValue > 0:
                ...
                self.cost[i] = 1
        sorted_arr = sorted(self.cost, reverse=True)
        a=1
    ...
    def setInilSolution_EAMC(self):
        ...
        #self.offSpringFit = np.mat(np.zeros([1, 4]))  # f(s),c(s),|s|,g(s)
...
def main(args):
    ...
    budget=args.budget
    times=args.times
    print(adjacency_file,budget,algo)
    ...
    if(args.ifDynamic):
        ...
        #dynamic result file
        res_file='dynamic-025kn/'+ args.adjacency_file + '_' + dynamic_budget_file + '_'+ algo
        if not os.path.exists(res_file):
            os.makedirs(res_file)

        warmUp_evaluateTime=int(args.n*(args.n+1)/2/4)
        dynamic_evaluateTime=int(args.n*(args.n+1)/2/2)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('-adjacency_file',type=str,default= "frb35-17-1.mis")
    argparser.add_argument('-q',type=int, default=6)
    argparser.add_argument('-n',type=int, default=450)
    argparser.add_argument('-ifDynamic',type=str2bool, default=True)
    argparser.add_argument('-algo',type=str, default="GGA") 
    argparser.add_argument('-times', type=int, default=0)
    argparser.add_argument('-budget',type=float, default=500)
    argparser.add_argument('-dynamic_budget_file',type=str,default= "dynamic_budget_MC.txt")
    argparser.add_argument('-budget_min',type=float, default=300)
    argparser.add_argument('-budget_max',type=float, default=700)
